// Read model do console em SQL. Só leitura; nada aqui muda estado.
import type pg from "pg";
import type { AgingBucket, ChargeFilter, ChargeRow, ConsoleQueries, ExceptionFilter, ExceptionRow, Page } from "../../core/console.js";
...
const excRow = (r: Row): ExceptionRow => ({
  ...
  refId: r.ref_id === null ? null : Number(r.ref_id), detail: r.detail, createdAt: iso(r.created_at)!, resolvedBy: (r.resolved_by as string) ?? null, resolvedAt: iso(r.resolved_at),
...
const chargeRow = (r: Row): ChargeRow => ({
  ...
  received: r.r_amount === null || r.r_amount === undefined ? null : { amountReceived: String(r.r_amount), paymentDate: (r.r_date as string) ?? null, diffPolicy: (r.r_policy as string) ?? null, odooPaymentId: r.r_odoo === null ? null : Number(r.r_odoo) },
...
export function createConsoleQueries(db: { query: pg.Pool["query"] }): ConsoleQueries {
  ...
  const clamp = (n: number | undefined, d: number, max: number) => Math.min(Math.max(n ?? d, 1), max);

  return {
    async exceptions(f: ExceptionFilter) {
      const limit = clamp(f.limit, 50, 200), offset = Math.max(f.offset ?? 0, 0);
      ...
      const rows = (await db.query(`${EXC_SELECT} ${w} order by e.status='open' desc, e.created_at desc limit ${limit} offset ${offset}`, params)).rows as Row[];
    ...
    async charges(f: ChargeFilter) {
      ...
      if (f.dueFrom) { params.push(f.dueFrom); where.push(`c.due_date >= $${params.length}`); }
      if (f.dueTo) { params.push(f.dueTo); where.push(`c.due_date <= $${params.length}`); }
      if (f.partnerId) { params.push(f.partnerId); where.push(`c.odoo_partner_id = $${params.length}`); }
      if (f.q) { params.push(`%${f.q}%`); where.push(`(c.invoice_name ilike $${params.length} or cm.name ilike $${params.length} or c.nosso_numero ilike $${params.length} or c.asaas_payment_id ilike $${params.length})`); }
      const w = where.length ? `where ${where.join(" and ")}` : "";
      ...
      const rows = (await db.query(`${CHARGE_SELECT} ${w} order by c.due_date asc, c.id asc limit ${limit} offset ${offset}`, params)).rows as Row[];
    ...
    async charge(id: number) {
      ...
      const reconciliations = (await db.query("select * from reconciliations where charge_id=$1 order by created_at", [id])).rows;
      ...
      const events = base.asaasPaymentId ? (await db.query("select id, asaas_event_id, event_type, process_status, received_at, processed_at, attempts, error from webhook_events where asaas_payment_id=$1 order by received_at", [base.asaasPaymentId])).rows : [];
    ...
    async aging(today: string) {
      const rows = (await db.query(`select case when due_date >= $1::date then 'a_vencer' when $1::date - due_date <= 7 then '1_7' when $1::date - due_date <= 30 then '8_30' else '31_mais' end as bucket,
        count(*)::int as count, coalesce(sum(amount),0)::text as amount from charges where status in ('created','confirmed') group by 1`, [today])).rows as Row[];
