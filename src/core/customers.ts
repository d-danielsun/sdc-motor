...
/** Garante o cliente no Asaas (reaproveita por externalReference; cria com notificações OFF até o R3). */
export async function ensureCustomer(deps: Deps, odooPartnerId: number): Promise<CustomerMap | null> {
  ...
  const base: Omit<CustomerMap, "id"> = {
    odooPartnerId, asaasCustomerId: null, cpfCnpj, name: partner.name, email: partner.email, phone: partner.phone,
  ...
  if (!cpfCnpj) {
    ...
    if (!(await repo.exceptions.hasOpen("customer_missing_document", "customers_map", odooPartnerId))) {
      await repo.exceptions.open({ type: "customer_missing_document", refTable: "customers_map", refId: odooPartnerId, detail: { partnerName: partner.name, vat: partner.vat } });
    }
    return null;
  }
  const ref = externalRefForPartner(odooPartnerId);
  const found = await asaas.findCustomerByExternalRef(ref);
  const customer = found ?? (await asaas.createCustomer({ name: partner.name, cpfCnpj, email: partner.email, phone: partner.phone, externalReference: ref, notificationDisabled: true }));
