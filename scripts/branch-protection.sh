#!/usr/bin/env bash
# Exige que ci/test, ci/build e gitleaks passem antes de qualquer merge na main.
#
...
if ! printf '%s' "$PAYLOAD" | gh api -X PUT "repos/$REPO/branches/$BRANCH/protection" --input - >/tmp/bp.$$ 2>/tmp/bp.err.$$; then
  ...
  if grep -qi "upgrade\|not available\|403" /tmp/bp.err.$$; then
    ...
    echo "(GitHub Pro/Team). Saídas: deixar o repo público, assinar o plano, ou usar" >&2
    echo "Rulesets pela interface. Enquanto isso o CI roda e sinaliza, mas não bloqueia." >&2
  fi
